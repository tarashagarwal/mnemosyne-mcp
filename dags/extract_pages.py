...
def extract_pdf_pages(
    ...
    """
    Extracts whole pages from a PDF using Docling and saves each page as a Markdown file.
    Pages are saved to temp_docs/pages/{book_name}/ folder.
    """
    ...
    if output_dir is None:
        output_dir = project_root / "temp_docs" / "pages" / book_name
    else:
        output_dir = Path(output_dir)
        if not output_dir.is_absolute():
            output_dir = project_root / output_dir
        # Ensure book_name subfolder exists
        if output_dir.name != book_name:
...
def main():
    ...
    pdf_path = project_root / "docs/the_once_and_future_queen.pdf"
    print(pdf_path)
    book_name = "the_once_and_future_queen"
    output_dir = project_root / "temp_docs" / "pages"
