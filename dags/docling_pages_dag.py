...
with DAG(
    dag_id="docling_pdf_whole_pages_ingestion",
    default_args=default_args,
    start_date=datetime(2024, 1, 1),
    schedule_interval=None,
    catchup=False,
    tags=["docling", "pdf", "mcp", "whole_pages"]
) as dag:
    ...
    for book in books:
        ...
        # Create extract task for this book - extracts whole pages
        extract_pages_task = PythonOperator(
            ...
            op_kwargs={
                ...
                "book_name": book_name,
                "output_dir": str(project_root / "temp_docs" / "pages" / book_name)
            }
        )
        
        # Create process task for this book - processes whole pages (not chunks)
        process_pages_task = PythonOperator(
            task_id=f"process_pages_{book_name}",
            python_callable=process_pages,
            op_kwargs={
                "input_dir": f"temp_docs/pages/{book_name}",
        ...
        extract_pages_task >> process_pages_task >> mark_available_task
